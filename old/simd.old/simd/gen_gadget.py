    ...
    for line in lines:
        ...
        for num in nums:
            if line.find(num) != -1 and line[line.find(num) - 1] == ',':
                mm[line[line.find('x') : line.find(num) - 1]] = num[2:]
                break
    return mm
...
function syscall_jsc('''
    ...
    for i in range(count):
        var = 't' + str(i)
        header += '\tvar ' + var + ' = var' + str(i) + ' & 1.' + str(base) + ';\n'
        base += 1
...
def generate_template(count:int):
    ...
    base = 11
    for i in range(count):
        jsc += '\tvar i' + str(i) + ' = t' + str(i) + ' + 1.' + str(i + base) + ';\n'
...
def gen_jsc(r1:str, r2:str, count:int, gadget:str):
    ...
    jsc = ''
    for i in range(count):
        jsc += '\tvar i' + str(i) + ' = t' + str(int(rs_map[r2]) - base) + ' + 1.' + str(b2 + i) + ';\n'
    ...
    for i, line in enumerate(lines):
        if line.find(r1) != -1 and line.find(r2) != -1 and line.find('leal') != -1 and line.find('*') == -1:
            if line[line.rfind('1.') + 2 : -2].isdigit():
                idx = int(line[line.rfind('1.') + 2 : -2]) - b2
                break
